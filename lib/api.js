...
function _resolveBaseFromWindow() {
  if (typeof window === "undefined") return ""; // SSR: same-origin on server
...
/** Fetch styles with extra cache-busting to ensure you always see the full list */
export async function fetchStyles() {
  const t = Date.now();
  const res = await apiGet(`/api/styles?t=${t}`, { noCache: true });
...
/** Some pages import { apiBase } — provide a constant for compatibility */
export const apiBase = getApiBase();

/** Optional default export */
export default { getApiBase, setApiBase, buildUrl, apiGet, apiPost, absoluteMediaUrl, fetchStyles, ping, apiBase };
